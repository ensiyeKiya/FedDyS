...
def get_fedavg_argparser() -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default="lenet5",
        choices=["lenet5", "2nn", "avgcnn", "mobile", "res18", "alex", "sqz", "rnn"],
    )
    parser.add_argument(
        "-d",
        "--dataset",
    ...
    parser.add_argument("--th_mode", type=str, default="mean")
...
class FedAvgServer:
    def __init__(
        ...
        if self.args.wandb:
            ...
            # in wandb.sh file set [wandb_api_key, STF_WANDB_PROJ, STF_WANDB_ACCT, wandb_log_dir]
            api_key = os.getenv("WANDB_API_KEY")
            wandb.login(key=api_key)

            log_dir = os.getenv("WANDB_LOG_DIR")
            print(
        ...
        self.client_trainable_params_deprecated: List[List[torch.Tensor]] = None
        ...
        if (
            ...
            for i in self.train_clients:
                self.set_client_trainable_params(i, trainable_params(self.model, detach=True))
        ...
        if default_trainer:
            self.trainer = FedAvgClient(
                deepcopy(self.model), self.args, self.logger, self.device, self.wandb_logger
            )

    def get_client_trainable_params(self, client_id):
        with open(f"tmp/{self.algo.lower()}_client{client_id}.pkl", 'rb') as file:
            return pickle.load(file)


    def set_client_trainable_params(self, client_id, client_trainable_params):
        with open(f"tmp/{self.algo.lower()}_client{client_id}.pkl", 'wb') as file:
            pickle.dump(client_trainable_params, file)
    ...
    def test(self):
        ...
        for client_id in self.test_clients:
            ...
            macro_f1_before.append(stats["before"]["macro_f1"])
            macro_f1_after.append(stats["after"]["macro_f1"])
        ...
        macro_f1_before = torch.FloatTensor(macro_f1_before)
        macro_f1_after = torch.FloatTensor(macro_f1_after)
        ...
        self.test_results[self.current_epoch + 1] = {
            ...
            "macro_f1": "{:.4f} -> {:.4f}".format(
                macro_f1_before.mean(),
                macro_f1_after.mean(),
            ),
        }
        self.test_flag = False
    ...
    def update_client_params(self, client_params_cache: List[List[torch.Tensor]]):
        ...
        if self.unique_model:
            for i, client_id in enumerate(self.selected_clients):
                self.set_client_trainable_params(client_id, client_params_cache[i])
    ...
    def generate_client_params(self, client_id: int) -> OrderedDict[str, torch.Tensor]:
        ...
        if self.unique_model:
            return OrderedDict(
                zip(self.trainable_params_name, self.get_client_trainable_params(client_id))
    ...
    def check_convergence(self):
        ...
        for label, metric in self.metrics.items():
            if label.startswith(("train", "test")):
                if len(metric) > 0:
    ...
    def log_info(self):
        ...
        metrics[f"macro_f1_before"].append(sum([self.client_stats[c][self.current_epoch]["before"]["macro_f1"].item()
                                                for c in self.selected_clients]) / len(self.selected_clients))
        metrics[f"macro_f1_after"].append(sum([self.client_stats[c][self.current_epoch]["after"]["macro_f1"].item()
                                               for c in self.selected_clients]) / len(self.selected_clients))
        ...
        for label in ["train", "test"]:
            ...
            if (label == "train" and self.args.eval_train) or (
                ...
                metrics[f"{label}_before"].append(acc_before)
                metrics[f"{label}_after"].append(acc_after)

                for k, v in metrics.items():
                    self.metrics[k].extend(v)
                self.logger.log(f"round metrics:{metrics}")

                if self.args.wandb:
                    self.wandb_logger.experiment.log({
                        "train_before": next(iter(metrics["train_before"]), None),
                        "train_after": next(iter(metrics["train_after"]), None),
                        "test_before": next(iter(metrics["test_before"]), None),
                        "test_after": next(iter(metrics["test_after"]), None),
                        "macro_f1_before": next(iter(metrics["macro_f1_before"]), None),
                        "macro_f1_after": next(iter(metrics["macro_f1_after"]), None)
